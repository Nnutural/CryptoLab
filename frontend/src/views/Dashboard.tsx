      ...
      try {
        ...
        if (!cancelled && resp.code === 1000 && Array.isArray(resp.data)) {
          ...
          if (rows.length) {
            ...
            setAvgLatency(total / rows.length);
          } else {
            setAvgLatency(0);
          }

          const head = rows.slice(0, 5).map((r: any) => ({
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
        ...
        <StatCard label="已存储密钥" value={keyCount} icon={<KeyRound size={18} />} />
        <StatCard label="近期操作次数" value={opCount} icon={<Activity size={18} />} />
        <StatCard label="平均响应延迟" value={avgLatency} decimals={2} unit="ms" icon={<Clock3 size={18} />} />
      </div>
